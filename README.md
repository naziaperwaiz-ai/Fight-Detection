...
**Hazard alerts are gated on the patient being unsupervised, and quiet hours are tracked but never used to hide anything.** Prompted by asking whether a dementia/Parkinson's patient's own caregiver picking up a knife next to them should page anyone -- it shouldn't. `pipeline.py` now computes `person_count` from the same per-frame `detections` list hazard detection already runs against, and treats `person_count <= 1` as "unsupervised." When `HAZARD_REQUIRE_UNSUPERVISED` (default `True`) is set and someone else is in frame, a hazard event is suppressed entirely -- no alert, no recorded clip, no drawn box -- on the reasoning that a second person present is implicitly supervising. This is deliberately narrower than real self-harm-risk or confused-possession detection: it doesn't reason about who the second person is or what they're doing, only whether anyone else is there at all, which is the most defensible signal available without inventing intent this project can't actually detect. `HAZARD_QUIET_HOURS_START`/`HAZARD_QUIET_HOURS_END` (both `None` by default, since there's no safe universal default) mark an unsupervised hazard event during that window as elevated-urgency in its alert detail -- they never gate whether the event fires, since a hazard alone in a room overnight is exactly the case this exists to catch, not one to suppress. Both settings started as `config.py`-only, no dashboard control at all; a System Settings → Hazard supervision card (admin-only, same "edited here, applied at the detection engine's next startup" pattern as Detection defaults above it) now lets an admin change them without hand-editing a file and restarting blind.

**A "Clip Ready" incident reports the confidence that triggered it, not a hardcoded 0%.** Every fired alert (Hazard, Violence, Fighting, Fall, Emergency) gets a saved clip, and once that clip finishes writing to disk, `_save_clip` logs a second, separate `"Clip Ready"` incident so a caregiver knows the recording is now viewable. That second incident used to always report `confidence: 0.0` regardless of what actually triggered the recording -- so a real 90%-confidence knife detection showed up in Incident History as a 0% entry sitting right next to the "Hazard Detected" incident that fired at 90%, reading as a failed detection when nothing had failed; it was just an unrelated bookkeeping notice reusing the wrong field. `_start_recording` now takes the triggering event's confidence (`avg_score` for Violence/Fighting, `detection_conf` for Hazard, the same `1.0` "rule fired, not a probability" convention Emergency/Fall's own alerts already use) and threads it through to `_save_clip`, so "Clip Ready" reports the real value. One known side effect worth naming rather than hiding: because recording restarts on sustained activity independent of the alert cooldown (see "Every alert type gets a clip" above), a single long event can still produce several "Clip Ready" entries with *different* confidence values -- one per 15-second recording chunk, each reflecting whatever the score was when that chunk's recording restarted -- which is more informative than the old flat 0%, but is not the same thing as one incident per real alert.
...
176 tests across 7 files, all passing:

`tests/test_dashboard.py` (68 tests) covers the Flask app end to end: login happy path and failure paths (wrong password, nonexistent email, both giving the same generic error), the login rate limit actually triggering a `429` after repeated attempts, the open-redirect guard on the post-login `next` parameter, session teardown on logout, the CORS-wildcard-stripping hook, admin-only enforcement on system settings and model upload (caregiver gets `403`, admin gets `200`), camera CRUD, internal-key enforcement on incident ingestion, incident notes/review/false-positive toggles, per-caregiver profile scoping (one caregiver's saved profile isn't visible as another's), round check-in logging and its same per-caregiver scoping, plus the admin-only check-in history endpoint (see "Quick actions are real actions" above), alert settings persistence (including the sound/desktop notification toggles), and analytics reflecting logged incidents. It also covers the invite flow specifically: a caregiver can't create invites (`403`), an admin-issued invite grants access to `/signup`, submitting it creates the account and logs the new user in, the same link is rejected as invalid on a second use, mismatched passwords are rejected without consuming the token, an invite can't target an email that already has an account, admins can revoke a pending invite, and repeated sign-up submissions trip the rate limiter. Room-scoped access gets its own block: an unassigned caregiver sees zero rooms, a scoped caregiver sees only their room across cameras/incidents/analytics/clips, cross-room video-feed/score/clip access is denied (including the deleted-camera edge case where a still-registered worker must not become reachable again), and only an admin can change a caregiver's room assignment. Retention gets its own block too: cleanup is admin-only, correctly deletes old and false-positive incidents on their respective windows, and the admin status endpoint reflects the last run. Two more tests cover that a hazard event's `person_count`/`quiet_hours` supervision context round-trips through `/api/events/add` into storage, and that non-hazard event types default both to `None` rather than a falsy `0`/`False` that would misleadingly read as "supervised" or "not quiet hours." Three more cover the Hazard supervision System Settings card: an admin can save `hazard_require_unsupervised`/quiet hours, quiet hours can be explicitly turned back off with a JSON `null` (not silently ignored or coerced to `0`, a real hour), and a caregiver gets `403` trying to write any of it. Each test runs against a throwaway store (SQLite events db plus the flat JSON stores), not real deployment data.

`tests/test_pipeline.py` (40 tests) covers `detection/pipeline.py`'s `process_frame()` alert-escalation and recording wiring: that Fighting/Fall/Emergency alerts dispatch (previously `has_fighting()` had zero callers anywhere in the codebase) and share one cooldown budget per camera rather than each getting its own; the live-feed privacy blur (frozen, pixelated placeholder except while something is actually happening -- Agitated and up, a confirmed fall, or a hazard event this frame -- with a hysteresis window before it re-blurs); hazard bounding-box visualization on the live feed; and clip recording for every alert type (Violence, Hazard, Fighting, Fall, Emergency all start a recording, an already-in-progress recording is never interrupted by a second event, and a clip is actually written to disk once `POST_EVENT_SECONDS` elapses). It also regression-tests `_dispatch_alert`'s and `_save_clip`'s SSL cert verification and error logging (see "Internal service authentication" above), `_save_clip`'s writer fps (derived from the real achieved capture rate, floored at a valid positive value, falling back to `config.FPS` only when the real elapsed span is missing or too small to trust -- see "A saved clip's frame rate is measured, not assumed" above), and hazard's supervision-context gating: a hazard event is suppressed when a second person is in frame, still fires when the patient is alone, still fires when supervised if `HAZARD_REQUIRE_UNSUPERVISED` is explicitly set `False`, and an unsupervised event is marked (via a monkeypatched clock) as occurring during quiet hours when configured and inside the window, and not marked when outside it. One more regression-tests that the "Clip Ready" incident `_save_clip` produces reports the real triggering detection's confidence (e.g. a hazard event's `detection_conf`), not the hardcoded `0.0` it used to -- see "A 'Clip Ready' incident reports the confidence that triggered it" below.
...
`tests/test_main.py` (16 tests) covers `src/main.py`'s `_build_camera_config`, the function that turns one `cameras.json` record into a per-camera `Config` used by `MultiCameraEngine` -- including that `DASHBOARD_CERT_PATH` is threaded through when TLS is configured and absent otherwise, and that the two hazard supervision-context System Settings (`hazard_require_unsupervised`, quiet hours) are picked up the same way `confirm_seconds`/`motion_threshold`/etc. already were, including that an explicit `None` for quiet hours survives the round trip as "off" rather than falling back to some other default.
...
| Hazard detection | fires per sampled frame once `HAZARD_MIN_CONSECUTIVE` consecutive detections are seen, and only while the patient is unsupervised (`person_count <= 1`) unless `HAZARD_REQUIRE_UNSUPERVISED` is set `False` | A COCO knife/scissors/fork-class object (detected in the same YOLO pass as person detection, at near-zero marginal cost) within `HAZARD_PROXIMITY_FRAC` of a pose-estimated wrist | `HAZARD_DETECTION_ENABLED`, `HAZARD_POSE_WEIGHTS`, `HAZARD_IMGSZ`, `HAZARD_SAMPLE_EVERY_N_FRAMES`, `HAZARD_PROXIMITY_FRAC`, `HAZARD_MIN_CONSECUTIVE`, `HAZARD_MIN_SEVERITY`, `HAZARD_REQUIRE_UNSUPERVISED`, `HAZARD_QUIET_HOURS_START`, `HAZARD_QUIET_HOURS_END` |

Hazard detection is opt-in and disabled by default (`HAZARD_DETECTION_ENABLED = False`) since the pose-estimation pass has a real CPU cost on top of the existing detection + classification pipeline. See "Hazard alerts are gated on the patient being unsupervised" in Key Design Decisions for why supervision context and quiet hours exist and how they differ (one suppresses, the other only marks urgency).
...
| `HAZARD_REQUIRE_UNSUPERVISED` | `True` | Suppresses hazard events entirely (no alert, no clip, no box) when someone else is also in frame -- set `False` to alert regardless of who else is present |
| `HAZARD_QUIET_HOURS_START` / `HAZARD_QUIET_HOURS_END` | `None` / `None` | 24-hour window (wraps past midnight) during which an unsupervised hazard event is marked elevated-urgency in its alert detail; both `None` disables this -- never gates whether the event fires |
